...
<script type="text/javascript">
	...
	function openGroupModifyDiglog(){
		...
		$("#fm").form("load",row);
		$("#flag").val(2);
		$("#abb").attr("readonly",true);
	}
	
	
	function checkData(){
		if($("#abb").val()===''){
			$.messager.alert("系统系统","请输入角色名缩写！");
			$("#abb").focus();
			return;
		}
		if($("#name").val()===''){
		...
		var flag=$("#flag").val();
		if(flag == 1){
			$.post("${pageContext.request.contextPath}/group/existGroupAbbName.do",{abb:$("#abb").val(),name:$("#name").val()},function(result){
				if(result.exist){
					$.messager.alert("系统系统","该角色名或缩写已存在，请更换下！");
					$("#abb").focus();
				}else{
					saveGroup();
				}
			...
			$.post("${pageContext.request.contextPath}/group/existGroupName.do",{name:$("#name").val()},function(result){
				if(result.exist){
...
<table id="dg" title="角色管理" class="easyui-datagrid"
 ...
 <thead>
 	<tr>
 		...
 		<th field="id" width="80" align="center">角色编号</th>
		<th field="abb" width="80" align="center">角色缩写</th>
 			...
 			<tr>
 				...
 				<td>
					<input type="hidden" id="id" name="id"/>
 					<input type="text" id="abb" name="abb" class="easyui-validatebox" required="true"/>
 				</td>
 				<td>&nbsp;&nbsp;&nbsp;&nbsp;</td>
 				<td>角色名称：</td>
 				<td>
